...
export class ThemeService {
  ...
  private readonly THEME_KEY = 'mylife-theme';
  ...
  configureDarkModeSwitch(element: ElementRef): void {
    const nativeElement = element.nativeElement;
    const iconOff = nativeElement.querySelector('.mdc-switch__icon--off');
    const iconOn = nativeElement.querySelector('.mdc-switch__icon--on');

    if (iconOff && iconOff.firstChild) {
      iconOff.firstChild.setAttribute('d', this.sun);
    }

    if (iconOn && iconOn.firstChild) {
      iconOn.firstChild.setAttribute('d', this.moon);
    }
